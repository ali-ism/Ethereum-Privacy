...
contract RingMixerV2 {
    ...
    function Deposit(address destination) payable public returns (bool success)
    {
    ...
    function DepositN(address[] memory destination) payable public returns (bool success)
    {
    ...
    function Withdraw(address[] memory destination, uint256[] memory value, uint256[] memory signature) public returns (bool success) 
    {
        ...
        for (i = 0; i < value.length; i++) {
            if (value[i] > txValue) return false; //Each value must be lesser than the balance //Check for crafty overflows
            temp += value[i];
        }
        if (temp != txValue) return false; //The sum of all values must be equal to the balance of a single address (because eventually) it will be coming from one address
    ...
    function GetAddress(uint256 PubKey) public view returns (address addr)
    {
    ...
    //Base EC Functions
    function ecAdd(uint256[2] memory p0, uint256[2] memory p1) public view returns (uint256[2] memory p2)
    {
        assembly {
    ...
    function ecMul(uint256[2] memory  p0, uint256 s) public view returns (uint256[2] memory p1)
    {
        assembly {
            //Get Free Memory Pointer
            let p := mload(0x40)
    ...
    function CompressPoint(uint256[2] memory Pin) public pure returns (uint256 Pout)
    {
        //Store x value
        Pout = Pin[0];
    ...
    function ExpandPoint(uint256 Pin) public view returns (uint256[2] memory Pout)
    {
    ...
    function HashFunction(RingMessage memory message, uint256[2] memory left, uint256[2] memory right) internal pure returns (uint256 h)
    {
    ...
    function HashPoint(uint256[2] memory p) internal view returns (uint256[2] memory h)
    {
        bool onCurve;
    ...
    function KeyImage(uint256 xk, uint256[2] memory Pk) internal view returns (uint256[2] memory Ix)
    {
        //Ix = xk * HashPoint(Pk)
        Ix = HashPoint(Pk);
        Ix = ecMul(Ix, xk);
    ...
    function RingStartingSegment(RingMessage memory message, uint256 alpha, uint256[2] memory P0) internal view returns (uint256 c0)
    {
        //Memory Registers
    ...
    function RingSegment(RingMessage memory message, uint256 c0, uint256 s0, uint256[2] memory P0, uint256[2] memory Ix) internal view returns (uint256 c1)
    {
        //Memory Registers
        uint256[2] memory temp;
        uint256[2] memory left;
    ...
    //SubMul = (alpha - c*xk) % N
    function SubMul(uint256 alpha, uint256 c, uint256 xk) internal pure returns (uint256 s)
    {
        s = mulmod(c, xk, N);
        s = N - s;
        s = addmod(alpha, s, N);        
    ...
    function RingVerify(RingMessage memory message, uint256[] memory signature) internal view returns (bool success)
    {
        //Check Array Lengths
    ...
    function RingVerify_User(address[] memory destination, uint256[] memory value, uint256[] memory signature) public view returns (bool success)
    {
